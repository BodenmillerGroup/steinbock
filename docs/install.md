...
For reproducibility, it is recommended to always pull a specific release, e.g.:
    ...
    docker pull jwindhager/steinbock:0.3.5
...
In the following instructions, instead of calling `steinbock`, type:
    ...
    docker run -v "C:\data":/data jwindhager/steinbock:0.3.5

In the command above, adapt the path to your *steinbock* data/working directory (`C:\data`) and the *steinbock* Docker container version (`0.3.5`) as needed.
...
Create an alias for running the *steinbock* Docker container:
    ...
    alias steinbock="docker run -v /mnt/data:/data -v /tmp/.X11-unix:/tmp/.X11-unix -v ~/.Xauthority:/root/.Xauthority:ro -e DISPLAY jwindhager/steinbock:0.3.5"

In the command above, adapt the path to your *steinbock* data/working directory (`/mnt/data`) and the *steinbock* Docker container version (`0.3.5`) as needed. The created alias enables running `steinbock` without typing the full Docker command everytime.
