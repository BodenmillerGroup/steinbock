...
!!! note "File permissions"
    ...
    However, if your user does not match the UID/GID (e.g. on multi-user environments, MacOS), you may not have ownership/write access to files generated by `steinbock`. In this case, you have the following options:
      ...
      - **[Workaround]** Recursively grant write access to the host system user matching UID `1000` and/or the host system group matching GID `1000` for the data/working directory and all files/directories below.

      - **[Linux only]** Add bind mounts for the `/etc/passwd` and `/etc/groups` files and run the Docker container with the current host system user, by supplying the following additional options to the `docker run` command above (remember to update your `steinbock` alias as well):

            -v /etc/passwd:/etc/passwd:ro -v /etc/groups:/etc/groups:ro -u $(id -u):$(id -g)

      - **[Singularity]** Run the *steinbock* Docker container using [singularity](https://sylabs.io/singularity/) (see [here](https://sylabs.io/guides/3.0/user-guide/singularity_and_docker.html)) to minimize abstraction

      - **[Custom build]** Re-build the *steinbock* Docker container from scratch with matching UID/GID ([Dockerfile](https://github.com/BodenmillerGroup/steinbock/blob/main/Dockerfile))

      - **[Contribute]** If you would like to provide a workaround for this inconvenience (e.g. automatically change the UID/GID at runtime of the *steinbock* Docker container), you are more than welcome to submit a pull request!

    The data/working directory must be writable by the user running within the *steinbock* Docker container.
