...
The second command runs the cellpose training module. Unless specified via the `--train_data` option, the command looks in `cellpose_crops` and `cellpose_masks` for images and masks that are used as ground truth. The resulting model is saved in the `training_out` directory.
