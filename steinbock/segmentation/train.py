...
def try_train_model(
  ...
  rescale: bool = True
    ...
    model = cellpose.models.CellposeModel(gpu=False, model_type= "tissuenet", net_avg=True)
    ...
    myOutput = model.train(train_data=train_images, train_labels=train_masks, train_files=None,
  ...
  rescale: bool = True
    ...
    for file in dir:
        ...
        if  str(Path(f).stem).startswith('.') == False:
            ...
            cellpose_crop_x, cellposek_crop_y, cellpose_crop = create_ilastik_crop (test_img, 250, rng)
            ...
            masks, flows, styles = model.eval(segstack, flow_threshold=1, cellprob_threshold=-6)
